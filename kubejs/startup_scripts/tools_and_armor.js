...
global.tiers = {
	...
	stone: {
		mod: "minecraft",
		knifeMod: "moredelight",
		material: "cosmic_contraptions:refined_stone",
		hasTools: true,
		hasArmor: false,
...
StartupEvents.registry("item", (event) => {
	for (const tierName in global.tiers) {
		...
		if (tier.hasTools) {
			...
			event
				.create("cosmic_contraptions:" + tierName + "_blade")
				.displayName(formatName(tierName) + " Blade")
				.unstackable();
			event
				.create("cosmic_contraptions:" + tierName + "_head")
			...
			if (!tier.doesntNeedUpgrading) {
				event
					.create("cosmic_contraptions:" + "incomplete_" + tierName + "_blade")
					.displayName("Incomplete " + formatName(tierName) + " Blade")
					.unstackable();
				event
					.create("cosmic_contraptions:" + "incomplete_" + tierName + "_head")
					.displayName("Incomplete " + formatName(tierName) + " Tool Head")
					.unstackable();
			}
			// Register Paxel
			event
				.create(`cosmic_contraptions:${tierName}_paxel`, "paxel")
			...
			if (!tier.hasOwnProperty("cannotBeBroken")) {
				tools.forEach((tool) => {
					event
						.create(`cosmic_contraptions:broken_${tierName}_${tool}`)
			...
			if (tier.hasOwnProperty("knifeMod")) {
				...
				if (!tier.hasOwnProperty("cannotBeBroken")) {
					event
						.create(`cosmic_contraptions:broken_${tierName}_knife`)
						.displayName("Broken " + formatName(tierName) + " Knife")
						.unstackable();
				}

				// If the knifeMod is "cosmic_contraptions", we need to create a knife
				if (tier.knifeMod === "cosmic_contraptions") {
					event
						.create("cosmic_contraptions:" + tierName + "_knife", "farmersdelight:knife")
		...
		if (tier.hasArmor) {
			...
			event
				.create("cosmic_contraptions:" + tierName + "_stitching")
			...
			if (!tier.doesntNeedUpgrading) {
				event
					.create("cosmic_contraptions:" + "unfinished_" + tierName + "_stitching")
			...
			if (!tier.hasOwnProperty("cannotBeBroken")) {
				armors.forEach((armor) => {
					event
						.create(`cosmic_contraptions:broken_${tierName}_${armor}`)
		...
		if (!tier.hasOwnProperty("cannotBeBroken")) {
			event
				.create("cosmic_contraptions:" + tierName + "_repair_kit")
...
ItemEvents.modification((event) => {
	for (const tierName in global.tiers) {
		...
		if (tier.hasTools) {
			event.modify(`cosmic_contraptions:${tierName}_paxel`, (item) => {
		...
		if (tier.hasOwnProperty("cannotBeBroken")) {
			tools.forEach((tool) => {
				event.modify(`cosmic_contraptions:${tierName}_${tool}`, (item) => {
					item.maxDamage = 0;
				});
			});
			armors.forEach((piece) => {
				event.modify(`cosmic_contraptions:${tierName}_${piece}`, (item) => {
					item.maxDamage = 0;
				});
			});
		}
